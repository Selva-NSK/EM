...
function addExpense(type){
	...
	var expenseBudgets = document.getElementsByName(type+"Budget");
	var expenseBudget = 'noBudgetRegd';
	for(var w=0; w<expenseBudgets.length; w++){
		if(expenseBudgets[w].checked!==false){
			expenseBudget = expenseBudgets[w].value;
		}
	}
	...
	if((expenseAmount == '') || (expenseDate == '') || (expenseCategory == '')){
		...
		var data = "addExpense="+addExpense+"&expenseAmount="+expenseAmount+"&expenseDate="+expenseDate+"&expenseCategory="+expenseCategory+"&expenseDetails="+expenseDetails+"&expenseUsername="+expenseUsername+"&expenseBudget="+expenseBudget+"&type="+type;
		/*Declare XML*/
		if(window.XMLHttpRequest){var xhr = new XMLHttpRequest();}
		else if(window.ActiveXObject){var xhr = new ActiveXObject("Microsoft.XMLHTTP");}
		/*AJAX Methods*/
		...
		xhr.onreadystatechange = function(){
			if(xhr.readyState == 4){
				if(xhr.status == 200){
					...
					if(eachResponse[0]=='noEnoughMoneyInBudget'){
						document.getElementById(type+"ErrorMessage").innerHTML = '<div class="errorMessageShake">No enough money in '+expenseBudget+'! <br>Select another Budget or update amount in Budget</div>';
					}else{
						...
						if(expenseBudget!='noBudgetRegd'){
							document.getElementById("allBudgetList").innerHTML=eachResponse[11];
							document.getElementById("BudgetHistory").innerHTML=eachResponse[12];
							/*Retain current value*/
							for(var wa=0; wa<expenseBudgets.length; wa++){
								expenseBudgets[wa].checked=false;
							}
						}
	...
	xhr.onreadystatechange = function(){
		if(xhr.readyState == 4){
			if(xhr.status == 200){
				...
				document.getElementById("editExpenseBudget"+expensesId).innerHTML = eachResponse[5];
			}
		}
	}
}
...
function confirmEdit(id){
	...
	var expensesBudget = document.getElementById("editExpenseBudget"+expensesId).value;
	/*Validation*/
	if((expensesAmount == '') || (expensesDate == '') || (expensesCategory == '')){
		...
		var data = "editExpenses="+editExpenses+"&expensesId="+expensesId+"&expensesAmount="+expensesAmount+"&expensesDate="+expensesDate+"&expensesCategory="+expensesCategory+"&expensesDetails="+expensesDetails+"&expensesBudget="+expensesBudget;
		/*Declare XML*/
		if(window.XMLHttpRequest){var xhr = new XMLHttpRequest();}
		else if(window.ActiveXObject){var xhr = new ActiveXObject("Microsoft.XMLHTTP");}
		/*AJAX Methods*/
...
/*Hide Edit Budget Div*/
function hideEditBudget(){
	document.getElementById("editBudgetDiv").innerHTML = "";
}

/*Hide Delete Budget Div*/
function hideDeleteBudget(){
	document.getElementById("deleteBudgetDiv").innerHTML = "";
}
...
function addBudget(){
	...
	var username = document.getElementById("username").value;
	document.getElementById("BudgetSubmit").innerHTML = "<div class='loaderButton'></div>";
	/*Validations*/
	if(BudgetName == '' || BudgetAmount == ''){ /*Empty Fields*/
		document.getElementById("BudgetErrorMessage").innerHTML = "<span class='errorMessageShake'>Please Fill in all the fields</span>";
		setTimeout(function(){document.getElementById("BudgetErrorMessage").innerHTML = "";},4000)
	}else{
		/*AJAX Functionality*/
		/*Declare variables*/
		var addBudget = "RandomInput";
		var data = "addBudget="+addBudget+"&BudgetName="+BudgetName+"&BudgetAmount="+BudgetAmount+"&username="+username;
		/*Declare XML*/
		if(window.XMLHttpRequest){var xhr = new XMLHttpRequest();}
		else if(window.ActiveXObject){var xhr = new ActiveXObject("Microsoft.XMLHTTP");}
		/*AJAX Methods*/
			...
			if(xhr.readyState == 4){
				if(xhr.status == 200){
					...
					if(eachResponse[0]=='success'){
						document.getElementById("BudgetErrorMessage").innerHTML = "<span class='successMessage'>Budget Added Successfully!</span>";
						setTimeout(function(){document.getElementById("BudgetErrorMessage").innerHTML = '';},4000);
						/*Set Budget list in add expense and income*/
						document.getElementById("expenseBudgetList").innerHTML=eachResponse[1];
						document.getElementById("incomeBudgetList").innerHTML=eachResponse[2];
						document.getElementById("allBudgetList").innerHTML=eachResponse[3];
						document.getElementById("addMoneyToBudget").innerHTML=eachResponse[4];
						/*Clearing inputs*/
						document.getElementById("BudgetName").value="";
						document.getElementById("BudgetAmount").value="";
					}else if(eachResponse[0]=='registeredAlready'){
						document.getElementById("BudgetErrorMessage").innerHTML = "<span class='errorMessageShake'>Budget with this name is already registered with us.. Try with a new name!</span>";
						setTimeout(function(){document.getElementById("BudgetErrorMessage").innerHTML = '';},4000);
					}else{
						document.getElementById("BudgetErrorMessage").innerHTML = "<span class='errorMessageShake'>Something went wrong!</span>";
						setTimeout(function(){document.getElementById("BudgetErrorMessage").innerHTML = '';},4000);
					}
	...
	document.getElementById("BudgetSubmit").innerHTML = "ADD";
}

/*Budget select*/
function setBudgetSelection(type){
	var username=document.getElementById("username").value;
	if(type=='credit'){var selectedValue=document.getElementById("BudgetCredit").value;}
	else if(type=='debit'){var selectedValue=document.getElementById("BudgetDebit").value;}
	/*AJAX Functionality*/
	/*Declare variables*/
	var setBudgetSelection = "RandomInput";
	var data = "setBudgetSelection="+setBudgetSelection+"&username="+username+"&selectedValue="+selectedValue;
	/*Declare XML*/
	if(window.XMLHttpRequest){var xhr = new XMLHttpRequest();}
	else if(window.ActiveXObject){var xhr = new ActiveXObject("Microsoft.XMLHTTP");}
	/*AJAX Methods*/
	...
	xhr.onreadystatechange = function(){
		if(xhr.readyState == 4){
			if(xhr.status == 200){
				if(type=='debit'){
					document.getElementById("BudgetCredit").innerHTML=this.responseText;
				}/*else if(type=='credit'){
					document.getElementById("BudgetDebit").innerHTML=this.responseText;
				}*/
			}
		}
	}
...
/*Budget Exhange Amount*/
function BudgetExchange(){
	var username=document.getElementById("username").value;
	var BudgetExchangeAmount=document.getElementById("BudgetExchangeAmount").value;
	var BudgetDebit=document.getElementById("BudgetDebit").value;
	var BudgetCredit=document.getElementById("BudgetCredit").value;
	var BudgetExchangeDate=document.getElementById("BudgetTransferDate").value;
	document.getElementById("BudgetExchangeSubmit").innerHTML = "<div class='loaderButton'></div>";
	/*Validations*/
	if(BudgetDebit == '' || BudgetCredit == '' || BudgetExchangeAmount == '' || BudgetExchangeDate == ''){ /*Empty Fields*/
		document.getElementById("BudgetExchangeErrorMessage").innerHTML = "<span class='errorMessageShake'>Please Fill in all the fields</span>";
		setTimeout(function(){document.getElementById("BudgetExchangeErrorMessage").innerHTML = "";},4000)
	}else{
		/*AJAX Functionality*/
		/*Declare variables*/
		var exchangeBudgetAmount = "RandomInput";
		var data = "exchangeBudgetAmount="+exchangeBudgetAmount+"&BudgetCredit="+BudgetCredit+"&BudgetDebit="+BudgetDebit+"&BudgetExchangeAmount="+BudgetExchangeAmount+"&BudgetExchangeDate="+BudgetExchangeDate+"&username="+username;
		/*Declare XML*/
		if(window.XMLHttpRequest){var xhr = new XMLHttpRequest();}
		else if(window.ActiveXObject){var xhr = new ActiveXObject("Microsoft.XMLHTTP");}
		/*AJAX Methods*/
			...
			if(xhr.readyState == 4){
				if(xhr.status == 200){
					...
					if(eachResponse[0]=='insufficientBalance'){
						document.getElementById("BudgetExchangeErrorMessage").innerHTML = "<span class='errorMessageShake'>Insufficient Balance in "+BudgetDebit+"</span>";
						setTimeout(function(){document.getElementById("BudgetExchangeErrorMessage").innerHTML = "";},4000)
					}else if(eachResponse[0]=='exchangeDone'){
						document.getElementById("BudgetExchangeErrorMessage").innerHTML = "<span class='successMessage'>&#8377;"+BudgetExchangeAmount+" added from "+BudgetDebit+" to "+BudgetCredit+" Successfully!</span>";
						setTimeout(function(){document.getElementById("BudgetExchangeErrorMessage").innerHTML = '';},4000);
						document.getElementById("allBudgetList").innerHTML=eachResponse[1];
						document.getElementById("BudgetHistory").innerHTML=eachResponse[2];
						/*Clearing Form*/
						document.getElementById("BudgetExchangeAmount").value="";
						document.getElementById("BudgetDebit").value="";
						document.getElementById("BudgetCredit").value="";
					}
				}
			}
		}
	}
	document.getElementById("BudgetExchangeSubmit").innerHTML = "SEND";
}


/*Show Budget Divs*/
function showBudgetDiv(type){
	closeBudgetDiv();
	document.getElementById(type).style.display="block";
	//Update new values while showing div
	if(type=="allBudgetList" || type=="BudgetHistory"){
		document.getElementById(type).innerHTML="<br><div class='loaderButton'></div><br>";
		var username=document.getElementById("username").value;
		/*AJAX Functionality*/
		/*Declare variables*/
		var updateBudgetDetails = "RandomInput";
		var data = "updateBudgetDetails="+updateBudgetDetails+"&username="+username;
		/*Declare XML*/
		if(window.XMLHttpRequest){var xhr = new XMLHttpRequest();}
		else if(window.ActiveXObject){var xhr = new ActiveXObject("Microsoft.XMLHTTP");}
		/*AJAX Methods*/
		...
		xhr.onreadystatechange = function(){
			if(xhr.readyState == 4){
				if(xhr.status == 200){
					eachResponse=this.responseText.split("-period-");
					if(type=='allBudgetList'){
						document.getElementById("allBudgetList").innerHTML=eachResponse[0];
					}
					else if(type=='BudgetHistory'){
						document.getElementById("BudgetHistory").innerHTML=eachResponse[1];
					}
				}
			}
		}
...
/*Close Budget Div*/
function closeBudgetDiv(){
	document.getElementById("addBudgetData").style.display="none";
	document.getElementById("allBudgetList").style.display="none";
	document.getElementById("addMoneyToBudget").style.display="none";
	document.getElementById("BudgetHistory").style.display="none";
}
...
function showEditBudget(BudgetId){
	/*AJAX Functionality*/
	/*Declare variables*/
	var showEditBudget = "RandomInput";
	var data = "showEditBudget="+showEditBudget+"&BudgetId="+BudgetId;
	document.getElementById("editBudgetDiv").style.display = "block";
	/*Declare XML*/
	if(window.XMLHttpRequest){var xhr = new XMLHttpRequest();}
	else if(window.ActiveXObject){var xhr = new ActiveXObject("Microsoft.XMLHTTP");}
	/*AJAX Methods*/
	...
	xhr.onreadystatechange = function(){
		if(xhr.readyState == 4){
			if(xhr.status == 200){
				eachResponse = this.responseText.split("-period-");
				document.getElementById("editBudgetDiv").innerHTML = eachResponse[0];
				document.getElementById("editBudgetName"+BudgetId).innerHTML = eachResponse[1];
				document.getElementById("editBudgetAmount"+BudgetId).innerHTML = eachResponse[2];
			}
		}
	}
}
...
/*Edit Budget*/
function confirmEditBudget(id,oldBudgetName){
	var BudgetId = id;
	var oldBudgetName=oldBudgetName;
	var BudgetName = document.getElementById("editBudgetName"+BudgetId).value;
	var BudgetAmount = document.getElementById("editBudgetAmount"+BudgetId).value;
	var username = document.getElementById("username").value;
	/*Validation*/
	if((BudgetAmount == '') || (BudgetName == '')){
		document.getElementById("editBudgetErrorMessage").innerHTML = "<span class = 'errorMessageShake'>Fill in the required fields.</span>";
	}else if(isNaN(BudgetAmount)){
		document.getElementById("editBudgetErrorMessage").innerHTML = "<span class = 'errorMessageShake'>Enter only numbers in amount.</span>";
	}else{
		/*AJAX Functionality*/
		/*Declare variables*/
		var editBudget = "RandomInput";
		var data = "editBudget="+editBudget+"&BudgetId="+BudgetId+"&BudgetAmount="+BudgetAmount+"&BudgetName="+BudgetName+"&username="+username+"&oldBudgetName="+oldBudgetName;
		/*Declare XML*/
		if(window.XMLHttpRequest){var xhr = new XMLHttpRequest();}
		else if(window.ActiveXObject){var xhr = new ActiveXObject("Microsoft.XMLHTTP");}
		/*AJAX Methods*/
		...
		xhr.onreadystatechange = function(){
			if(xhr.readyState == 4){
				if(xhr.status == 200){
					document.getElementById("editBudgetErrorMessage").innerHTML = "<span class = 'successMessage'>Details Edited</span>";
					setTimeout(function(){document.getElementById("editBudgetErrorMessage").innerHTML = "<div class = 'loaderButton'></div>";},500)
					setTimeout(function(){document.getElementById("editBudgetDiv").innerHTML = "";},1000);
					var eachResponse = this.responseText.split("-period-");
					document.getElementById("allBudgetList").innerHTML=eachResponse[1];
					document.getElementById("addMoneyToBudget").innerHTML=eachResponse[2];
					document.getElementById("expenseBudgetList").innerHTML=eachResponse[3];
					document.getElementById("incomeBudgetList").innerHTML=eachResponse[4];
				}
			}
		}
	}
...
/*Show delete Budget*/
function showDeleteBudget(id){
	var BudgetId = id;
	/*AJAX Functionality*/
	/*Declare variables*/
	var showDeleteBudget = "RandomInput";
	var data = "showDeleteBudget="+showDeleteBudget+"&BudgetId="+BudgetId;
	document.getElementById("deleteBudgetDiv").style.display = "block";
	/*Declare XML*/
	if(window.XMLHttpRequest){var xhr = new XMLHttpRequest();}
	else if(window.ActiveXObject){var xhr = new ActiveXObject("Microsoft.XMLHTTP");}
	/*AJAX Methods*/
	...
	xhr.onreadystatechange = function(){
		if(xhr.readyState == 4){
			if(xhr.status == 200){
				document.getElementById("deleteBudgetDiv").innerHTML = this.responseText;
			}
		}
	}
}
...
/*Delete Budget*/
function confirmDeleteBudget(id,BudgetName){
	var BudgetId = id;
	var username=document.getElementById("username").value;
	/*AJAX Functionality*/
	/*Declare variables*/
	var deleteBudget = "RandomInput";
	var data = "deleteBudget="+deleteBudget+"&BudgetId="+BudgetId+"&username="+username+"&BudgetName="+BudgetName;
	/*Declare XML*/
	if(window.XMLHttpRequest){var xhr = new XMLHttpRequest();}
	else if(window.ActiveXObject){var xhr = new ActiveXObject("Microsoft.XMLHTTP");}
	/*AJAX Methods*/
	...
	xhr.onreadystatechange = function(){
		if(xhr.readyState == 4){
			if(xhr.status == 200){
				document.getElementById("deleteBudgetErrorMessage").innerHTML = "<span class = 'successMessage'>Budget Deleted Successfully!</span>";
				setTimeout(function(){document.getElementById("deleteBudgetErrorMessage").innerHTML = "<div class = 'loaderButton'></div>";},500)
				setTimeout(function(){document.getElementById("deleteBudgetDiv").innerHTML = "";},1000);
				/*Update New List*/
				var eachResponse = this.responseText.split("-period-");
				document.getElementById("allBudgetList").innerHTML=eachResponse[0];
				document.getElementById("addMoneyToBudget").innerHTML=eachResponse[1];
				document.getElementById("expenseBudgetList").innerHTML=eachResponse[2];
				document.getElementById("incomeBudgetList").innerHTML=eachResponse[3];
			}
		}
	}
}
...
function deleteBudgetHistory(id){
	var BudgetId = id;
	/*AJAX Functionality*/
	/*Declare variables*/
	var showDeleteBudgetHistory = "RandomInput";
	var data = "showDeleteBudgetHistory="+showDeleteBudgetHistory+"&BudgetId="+BudgetId;
	document.getElementById("deleteBudgetDiv").style.display = "block";
	/*Declare XML*/
	if(window.XMLHttpRequest){var xhr = new XMLHttpRequest();}
	else if(window.ActiveXObject){var xhr = new ActiveXObject("Microsoft.XMLHTTP");}
...
function confirmDeleteBudgetHistory(id){
	var BudgetId = id;
	var username=document.getElementById("username").value;
	/*AJAX Functionality*/
	/*Declare variables*/
	var deleteBudgetHistory = "RandomInput";
	var data = "deleteBudgetHistory="+deleteBudgetHistory+"&BudgetId="+BudgetId+"&username="+username;
	/*Declare XML*/
	if(window.XMLHttpRequest){var xhr = new XMLHttpRequest();}
	else if(window.ActiveXObject){var xhr = new ActiveXObject("Microsoft.XMLHTTP");}
	/*AJAX Methods*/
	...
	xhr.onreadystatechange = function(){
		if(xhr.readyState == 4){
			if(xhr.status == 200){
				document.getElementById("deleteBudgetErrorMessage").innerHTML = "<span class = 'successMessage'>Transaction Deleted Successfully!</span>";
				setTimeout(function(){document.getElementById("deleteBudgetErrorMessage").innerHTML = "<div class = 'loaderButton'></div>";},500)
				setTimeout(function(){document.getElementById("deleteBudgetDiv").innerHTML = "";},1000);
				/*Update New List*/
				var eachResponse = this.responseText.split("-period-");
				...
				document.getElementById("expenseBudgetList").innerHTML=eachResponse[2];
				document.getElementById("incomeBudgetList").innerHTML=eachResponse[3];
				document.getElementById("BudgetHistory").innerHTML=eachResponse[4];
			}
		}
	}
}
...
function editBudgetHistory(id) {
	var BudgetId = id;
	/*AJAX Functionality*/
	/*Declare variables*/
	var showEditBudgetHistory = "RandomInput";
	var data = "showEditBudgetHistory=" + showEditBudgetHistory + "&BudgetId=" + BudgetId;
	document.getElementById("editBudgetDiv").style.display = "block";
	/*Declare XML*/
	if (window.XMLHttpRequest) { var xhr = new XMLHttpRequest(); }
	else if (window.ActiveXObject) { var xhr = new ActiveXObject("Microsoft.XMLHTTP"); }
	...
	xhr.onreadystatechange = function () {
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				eachResponse = this.responseText.split("-period-");
				document.getElementById("editBudgetDiv").innerHTML = eachResponse[0];
				document.getElementById("editBudgetHistoryFrom" + BudgetId).innerHTML = eachResponse[1];
				if (document.getElementById("editBudgetHistoryTo" + BudgetId)) { document.getElementById("editBudgetHistoryTo" + BudgetId).innerHTML = eachResponse[2]; }
				document.getElementById("editBudgetHistoryValue" + BudgetId).innerHTML = eachResponse[3];
				document.getElementById("editBudgetHistoryDate" + BudgetId).innerHTML = eachResponse[4];
				if (document.getElementById("editBudgetHistoryType" + BudgetId)) { document.getElementById("editBudgetHistoryType" + BudgetId).innerHTML = eachResponse[5]; }
				if (document.getElementById("editBudgetHistoryCategory" + BudgetId)) { document.getElementById("editBudgetHistoryCategory" + BudgetId).innerHTML = eachResponse[6]; }
				if (document.getElementById("editBudgetHistoryDetails" + BudgetId)) { document.getElementById("editBudgetHistoryDetails" + BudgetId).innerHTML = eachResponse[7]; }
			}
		}
	}
}
...
/*Edit Budget History*/
function confirmEditBudgetHistory(id) {
	var BudgetId = id;
	var username = document.getElementById("username").value;
	var BudgetHistoryFrom = document.getElementById("editBudgetHistoryFrom" + BudgetId).value;
	if (document.getElementById("editBudgetHistoryTo" + BudgetId)) {
		var BudgetHistoryTo = document.getElementById("editBudgetHistoryTo" + BudgetId).value;
	} else { var BudgetHistoryTo = 'BudgetExpenseOK';}
	var BudgetHistoryValue = document.getElementById("editBudgetHistoryValue" + BudgetId).value;
	var BudgetHistoryDate = document.getElementById("editBudgetHistoryDate" + BudgetId).value;
	if (document.getElementById("editBudgetHistoryType" + BudgetId)) {
		var BudgetHistoryType = document.getElementById("editBudgetHistoryType" + BudgetId).value;
	} else { var BudgetHistoryType = 'BudgetTransfer';}
	if (document.getElementById("editBudgetHistoryCategory" + BudgetId)) {
		var BudgetHistoryCategory = document.getElementById("editBudgetHistoryCategory" + BudgetId).value;
	} else { var BudgetHistoryCategory = 'BudgetTransfer';}
	if (document.getElementById("editBudgetHistoryDetails" + BudgetId)) {
		var BudgetHistoryDetails = document.getElementById("editBudgetHistoryDetails" + BudgetId).value;
	} else { var BudgetHistoryDetails = 'BudgetTransfer';}
	/*Validation*/
	if ((BudgetHistoryFrom == '') || (BudgetHistoryValue == '') || (BudgetHistoryDate == '')) {
		document.getElementById("editBudgetErrorMessage").innerHTML = "<span class = 'errorMessageShake'>Fill in the required fields.</span>";
	} else if (isNaN(BudgetHistoryValue)) {
		document.getElementById("editExpenseErrorMessage").innerHTML = "<span class = 'errorMessageShake'>Enter only numbers in amount.</span>";
	} else if (BudgetHistoryDate.match(/^\d{2}-\d{2}-\d{4}$/) === null) {
		document.getElementById("editExpenseErrorMessage").innerHTML = "<span class = 'errorMessageShake'>Please Enter Date in dd-mm-yyyy format only.</span>";
	} else {
		/*New Budget Value*/
		var currentOldBudgetValue = document.getElementById("currentBudgetValue" + BudgetId).value;
		var bufferBudgetValue = currentOldBudgetValue - BudgetHistoryValue;
		/*AJAX Functionality*/
		/*Declare variables*/
		var editBudgetHistory = "RandomInput";
		var data = "editBudgetHistory=" + editBudgetHistory + "&username=" + username + "&BudgetId=" + BudgetId + "&BudgetHistoryFrom=" + BudgetHistoryFrom + "&BudgetHistoryTo=" + BudgetHistoryTo + "&BudgetHistoryValue=" + BudgetHistoryValue + "&BudgetHistoryDate=" + BudgetHistoryDate + "&BudgetHistoryType=" + BudgetHistoryType + "&BudgetHistoryCategory=" + BudgetHistoryCategory + "&BudgetHistoryDetails=" + BudgetHistoryDetails + "&bufferBudgetValue=" + bufferBudgetValue;
		/*Declare XML*/
		if (window.XMLHttpRequest) { var xhr = new XMLHttpRequest(); }
		else if (window.ActiveXObject) { var xhr = new ActiveXObject("Microsoft.XMLHTTP"); }
		/*AJAX Methods*/
...
function showBudgetButtons(){
	if(document.getElementById("BudgetButtons").style.display=="block"){
		document.getElementById("BudgetButtons").style.display="none";
		closeBudgetDiv();
		document.getElementById("BudgetActionDiv").innerHTML="<button type='button' id='showBudgetButtons' class='basicButtonOuter smallButton' onclick='showBudgetButtons()'>Budget Details</button>";
	}else{
		document.getElementById("BudgetButtons").style.display="block";
		showBudgetDiv("allBudgetList");
		document.getElementById("BudgetActionDiv").innerHTML="<button type='button' class='redButton' onclick='showBudgetButtons()'>CLOSE</button>";
	}
}
...
//Filter Budget
function filterBudgetHistory(Budget){
	var allRows=document.getElementsByClassName('allBudgetHistoryRows');
	if(Budget!='all'){
		for(var i = 0; i < allRows.length; i++){
			allRows[i].style.display = "none";
		}
		//Display Required Budget Details
		var reqRows=document.getElementsByClassName(Budget+"BudgetDetails");
