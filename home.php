...
<body>
	...
	if(isset($_SESSION['id'])){
		...
		echo '<div id="header">
			...
			<div id="headerName"';?> onclick = "window.location.href = 'home.php'" <?php echo '><span class="headerIconShow">$</span> Budgetify <span class="headerIconShow">&#9998;</span></div>
		...
		<div id = "content">
			...
			<div id="utilityDiv">
				...
				<a href="src/plaid-link.html">
            <button class="searchButton filterButton" >Link To Bank Account</button>
        </a>
			...
			<div id="allDetailsDiv">
				...
				<div id="editBudgetDiv"></div>
				<div id="deleteBudgetDiv"></div>
			...
			<?php echo '<div id = "contentDashboard">
				...
				echo '<div id="marqueeDashboard">';
					...
					$getThisYearExpense = mysqli_query($conn,"SELECT SUM(amount) AS thisYearExpense FROM expenses WHERE type = 'expense' AND category != 'BudgetTransfer' AND username = '$username' AND YEAR(date) = '$thisYear' ORDER BY id DESC");
					$getThisYearExpenseCount = mysqli_query($conn,"SELECT * FROM expenses WHERE type = 'expense' AND category != 'BudgetTransfer' AND username = '$username' AND YEAR(date) = '$thisYear' ORDER BY id DESC");
					...
					$getThisYearIncome = mysqli_query($conn,"SELECT SUM(amount) AS thisYearIncome FROM expenses WHERE type = 'income' AND category != 'BudgetTransfer' AND username = '$username' AND YEAR(date) = '$thisYear' ORDER BY id DESC");
					$getThisYearIncomeCount = mysqli_query($conn,"SELECT * FROM expenses WHERE type = 'income' AND category != 'BudgetTransfer' AND username = '$username' AND YEAR(date) = '$thisYear' ORDER BY id DESC");
					...
					echo '
					<div id="BudgetActionDiv">
						<button type="button" id="showBudgetButtons" class="basicButtonOuter smallButton" onclick="showBudgetButtons()">Budget Details</button>';
						/*Update for adding data from Budgethistory to expense*/
					...
					echo'</div>
					<div class="flexDisplay" id="BudgetButtons">
						<button type="button" id="addBudgetButton" class="greenButtonOuter smallButton" onclick="showBudgetDiv(\''."addBudgetData".'\')">Add New Budget</button>
						<button type="button" id="addBudgetMoneyButton" class="greenButtonOuter smallButton" onclick="showBudgetDiv(\''."addMoneyToBudget".'\')">Transfer Money to Budget</button>
						<button type="button" id="showBudgetDataButton" class="basicButtonOuter smallButton" onclick="showBudgetDiv(\''."allBudgetList".'\')">Show Budget Amount</button>
						<button type="button" id="BudgetHistoryButton" class="greenButtonOuter smallButton" onclick="showBudgetDiv(\''."BudgetHistory".'\')">Budget History</button>
					</div>
					<div id="addBudgetData">
						<br><u>Add New Budget:</u><br><br>
						<div>Month  : <input id="BudgetName" type="text" class="inputStyle" placeholder="(Eg : Dec,June)"></div>
						<div>Budget Amount : <div class="inlineBlockDiv">&#8377;<input id="BudgetAmount" type="number" class="inputStyle" placeholder="Amount"></div></div>
						<div id="BudgetErrorMessage"></div>
						<div>
							<button type="button" id="BudgetSubmit" class="submitButton" onclick="addBudget()">ADD</button>
							<button type="button" class="closeButton" onclick="showBudgetButtons()">CLOSE</button>
						</div><br>';
					echo'<br></div>
					<div id="allBudgetList">';
						$BudgetCheck=mysqli_query($conn,"SELECT * FROM Budget WHERE BudgetUsername='$username'");
					echo '</div>';
					/*Budget amount transfers*/
					echo'<div id="addMoneyToBudget">';
						if(mysqli_num_rows($BudgetCheck)>1){
							...
							<button type="button" class="closeButton" onclick="showBudgetButtons()">CLOSE</button><br><br>';
						}else{
							if(mysqli_num_rows($BudgetCheck)<1){echo 'No Budgets registered.';}
							else{echo'<br>Minimum of 2 Budgets required to transfer amount!<br><br>';}
						}
					echo'</div>';
					/*Budget History*/
					...
					<span id="expenseFormOtherDataShow">
						...
						if(mysqli_num_rows($getBudget)>0){
							while($rowBudget=mysqli_fetch_assoc($getBudget)){
								...
								echo '<span class="inlineBlockDiv"><input type="radio" id="expenseBudget'.$BudgetNameTrim.'" class="inputStyle" name="expenseBudget" value="'.$rowBudget['BudgetName'].'"><label for="expenseBudget'.$BudgetNameTrim.'">'.$rowBudget['BudgetName'].'</label></span>';
							}
							echo'<span class="inlineBlockDiv"><input type="radio" id="expenseBudgetCash" class="inputStyle" name="expenseBudget" value="cash"><label for="expenseBudgetCash">Others</label></span>
							<span class="inlineBlockDiv"><a href="#marquee" type="button" class="greenButtonOuter smallButton" onclick="showBudgetDiv(\''."addBudgetData".'\')">New Budget</a></span>';
						}
						echo'</div>';
						?>
			...
			<div id = "contentIncomeForm">
				...
				<div id="incomeForm">
					...
					<span id="incomeFormOtherDataShow">
						...
						<?php
						echo'<div id="incomeBudgetList">';
						$getBudget=mysqli_query($conn,"SELECT * FROM Budget WHERE BudgetUsername='$username'");
						if(mysqli_num_rows($getBudget)>0){
							while($rowBudget=mysqli_fetch_assoc($getBudget)){
								$BudgetNameTrim=trim($rowBudget['BudgetName']);
								echo '<span class="inlineBlockDiv"><input type="radio" id="incomeBudget'.$BudgetNameTrim.'" class="inputStyle" name="incomeBudget" value="'.$rowBudget['BudgetName'].'"><label for="incomeBudget'.$BudgetNameTrim.'">'.$rowBudget['BudgetName'].'</label></span>';
							}
							echo'<span class="inlineBlockDiv"><input type="radio" id="incomeBudgetCash" class="inputStyle" name="incomeBudget" value="Cash"><label for="incomeBudgetCash">Others</label></span>
							<a href="#marquee" type="button" class="greenButtonOuter smallButton" onclick="showBudgetDiv(\''."addBudgetData".'\')">New Budget</a>';
						}
						echo'</div>';
						?>
			...
			<div id = "contentShortSummaryForm">
				...
				/*Get all Expenses*/
				$getExpense = mysqli_query($conn,"SELECT * FROM expenses WHERE type = 'expense' AND category != 'BudgetTransfer' AND username = '$username' ORDER BY id DESC");
				$totalExpenses = 0;
				...
				/*Get all Incomes*/
				$getIncome = mysqli_query($conn,"SELECT * FROM expenses WHERE type = 'income' AND category != 'BudgetTransfer' AND username = '$username' ORDER BY id DESC");
				$totalIncome = 0;
			...
			<?php
				...
				/*Data Accumulation*/
				/*Get each month*/
				$getEachExpenseMonth = mysqli_query($conn, "SELECT DISTINCT month(date) AS month, year(date) AS year FROM expenses WHERE username = '$username' AND type='expense' AND category != 'BudgetTransfer' ORDER BY date DESC");
				if(mysqli_num_rows($getEachExpenseMonth)>0){
					...
					while($EachExpenseMonth = mysqli_fetch_assoc($getEachExpenseMonth)){
						...
						$getMonthExpenses=mysqli_query($conn,"SELECT * FROM expenses WHERE month(date)='$monthOfGraph' AND year(date)='$yearOfGraph' AND username = '$username' AND type='expense' AND category != 'BudgetTransfer' ORDER BY date ASC");
						if(mysqli_num_rows($getMonthExpenses)>0){$dateCheck=1;$largestExpenseInMonth=0;$largestExpenseInMonthSpecificDate=0;$averageExpenseInMonthTotal=0;$averageExpenseInMonth=0;$averageExpenseInMonthCount=1;
							...
							/*Overall Expense*/
							$getOverallExpenseInMonth=mysqli_query($conn,"SELECT SUM(amount) AS totalExpenseAmount FROM expenses WHERE username = '$username' AND type = 'expense' AND category != 'BudgetTransfer' AND month(date) = '$monthOfGraph' AND year(date) = '$yearOfGraph'");
